...
import { UserPlus, Trash2, UserCircle, Camera, Upload } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';

const EMOJIS = ['🏐', '🔥', '⭐️', '👑', '🦁', '🦅', '⚡️', '😎', '💪', '🧤', '🎯', '🚀'];

export default function PlayerManager({ players, onAdd, onDelete }) {
    ...
    return (
        <div className="space-y-8 pb-24 text-white">
            <header className="space-y-1">
                <h2 className="text-3xl font-black italic tracking-tighter uppercase text-white">成員 <span className="text-emerald-400">JOIN</span></h2>
                <p className="text-gray-500 text-sm font-bold">同事請在此輸入名字並上傳相片或揀選 Icon 加入。</p>
            </header>

            {/* Self-Registration Form */}
            <section className="p-6 glass rounded-[40px] border border-emerald-500/20 bg-emerald-500/5 relative overflow-hidden">
                ...
                <form onSubmit={handleSubmit} className="space-y-6 relative z-10">
                    <div className="space-y-3">
                        <label className="text-[10px] font-black text-emerald-400 uppercase tracking-widest ml-1">1. 選擇你的 ICON 或上傳相片</label>
                        ...
                        <div className="flex flex-wrap gap-2.5">
                            ...
                            <button
                                ...
                                className={`w-11 h-11 rounded-2xl flex items-center justify-center text-xl transition-all active:scale-90 border-2 border-dashed ${selectedIcon.startsWith('data:image')
                            ...
                            {EMOJIS.map(emoji => (
                                <button
                                    ...
                                    className={`w-11 h-11 rounded-2xl flex items-center justify-center text-2xl transition-all active:scale-90 ${selectedIcon === emoji
                                            ? 'bg-emerald-500 text-white shadow-xl shadow-emerald-500/30 ring-2 ring-emerald-400 ring-offset-4 ring-offset-background'
                    ...
                    <div className="space-y-3">
                        <label className="text-[10px] font-black text-emerald-400 uppercase tracking-widest ml-1">2. 輸入你的大名</label>
                ...
                <div className="grid grid-cols-1 gap-3">
                    <AnimatePresence mode="popLayout">
                        {players.map((p) => (
                            <motion.div
                                ...
                                key={p.id}
                                className="p-5 glass rounded-3xl flex items-center justify-between border border-white/5 hover:border-white/10 transition-all group"
                            >
                    ...
                    {players.length === 0 && (
                        <motion.div
                            initial={{ opacity: 0 }}
                            animate={{ opacity: 1 }}
                            className="p-20 text-center space-y-3"
                        >
                            <div className="w-20 h-20 bg-white/5 rounded-full flex items-center justify-center mx-auto text-gray-800">
                                <UserPlus className="w-10 h-10" />
                            </div>
                            <p className="text-gray-600 font-bold italic">仲未有成員加入，快啲做第一個啦！</p>
                        </motion.div>
                    )}
                </div>
            </section>
